...
import { Check, X, Star, Flame, CheckCircle2 } from 'lucide-react';
...
export const TeamOperatorConsole: React.FC<TeamConsoleProps> = ({ teamId, align }) => {
  ...
  return (
    <motion.aside
      ...
      className="w-[280px] sm:w-[300px] md:w-[320px] pointer-events-auto flex flex-col justify-between p-3 rounded-2xl bg-[#fef08a] border-4 border-black select-none shrink-0 shadow-[6px_6px_0px_#000000] z-30 max-h-[calc(100vh-80px)] overflow-y-auto"
    >
      ...
      {/* ── 2. Challenge Prompt & Goal (Contained Box) ── */}
      {activeChallenge && (
        <div className="my-2 p-2.5 rounded-xl bg-white border-3 border-black shadow-[3px_3px_0px_#000000] text-left">
      ...
      {/* ── 3. Tactile Neo-Brutalist Choice Cards ── */}
        ...
        {activeChallenge?.choices.map((choice) => {
          ...
          return (
            <button
              ...
              className={`w-full min-h-[56px] sm:min-h-[62px] p-2 sm:p-2.5 rounded-xl border-3 text-left font-black transition-all flex items-center justify-between gap-2.5 touch-manipulation cursor-pointer ${cardClasses} ${
      ...
      <div className="mt-1.5 pt-1.5 border-t-2 border-black/20">
