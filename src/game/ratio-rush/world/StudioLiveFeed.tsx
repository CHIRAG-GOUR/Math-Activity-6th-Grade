...
//
// Every screen on this stage (both camera viewfinders, the director's
// video-village monitor and the premiere projection screen) shows what
// the A-camera is actually pointing at, rendered for real from the film
// camera's position into a render target each frame — not a painted
// picture of a stage.
//
...
/** Where the A-camera is parked, and what it is framed on. */
const CAM_POS = new THREE.Vector3(1.4, 1.66, 3.2);
const CAM_TARGET = new THREE.Vector3(-0.1, 1.2, -2.25);
...
export const StudioLiveFeedProvider: React.FC<{
  ...
  const target = useMemo(() => {
    ...
    return new THREE.WebGLRenderTarget(768, 432, {
  ...
  const value = useMemo<LiveFeed>(() => ({ screenMaterial, texture: target.texture }), [screenMaterial, target]);
  ...
  useFrame(({ gl, scene, clock }) => {
    ...
    if (isFilming) {
      feedCam.position.set(
        CAM_POS.x + Math.sin(t * 1.7) * 0.035,
        CAM_POS.y + Math.cos(t * 1.3) * 0.022,
        CAM_POS.z
      );
      lookAt.current.set(
        CAM_TARGET.x + Math.sin(t * 0.7) * 0.28,
        CAM_TARGET.y + Math.cos(t * 0.9) * 0.06,
        CAM_TARGET.z
      );
    } else {
