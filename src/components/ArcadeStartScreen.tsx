...
import { Sparkles, Play, Volume2, VolumeX, Swords } from 'lucide-react';
...
export const ArcadeStartScreen: React.FC<ArcadeStartScreenProps> = ({
  settings,
  onUpdateSettings,
  onStartGame,
}) => {
  ...
  return (
    <div className="relative w-full h-full flex flex-col items-center justify-between p-4 sm:p-6 lg:p-8 z-30 select-none max-w-7xl mx-auto overflow-y-auto">
      {/* Top Header Badge & Sound Button */}
      <div className="w-full flex items-center justify-between">
        <div className="flex items-center gap-2 px-4 py-1.5 rounded-full bg-slate-950/90 border border-amber-400/70 shadow-[0_0_18px_rgba(255,215,0,0.4)]">
        ...
        <button
          onClick={toggleSound}
          className="tactile-btn p-2.5 rounded-2xl bg-slate-950/90 border border-slate-700 text-slate-300 hover:text-white transition shadow-lg cursor-pointer"
        >
      ...
      >
        <div className="relative flex items-center justify-center">
          <div className="absolute w-60 h-60 rounded-full bg-amber-500/25 blur-3xl pointer-events-none" />
          <h1 className="text-6xl sm:text-7xl md:text-8xl lg:text-9xl font-black tracking-tight gold-metallic-text font-display uppercase drop-shadow-[0_12px_40px_rgba(0,0,0,1)]">
            MATH VAULT
          </h1>
        </div>
      ...
      {/* 4 Arcade Game Setup Panels */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 w-full my-4">
        
        {/* 1. TARGET GRADE */}
        <div className="chamfer-hud-center p-4 rounded-3xl flex flex-col gap-2.5">
          ...
          <div className="grid grid-cols-5 gap-1.5">
            {grades.map((g) => (
              <button
                ...
                className={`tactile-btn py-2 px-1 rounded-xl text-xs font-black font-game border-2 transition-all cursor-pointer ${
                  settings.grade === g
          ...
          <div className="grid grid-cols-2 gap-1.5 max-h-36 overflow-y-auto pr-1">
            {topics.map((t) => (
              <button
                ...
                className={`tactile-btn py-2 px-2 rounded-xl text-[11px] font-black font-game border-2 flex items-center gap-1.5 transition-all cursor-pointer ${
                  settings.topic === t.id
                    ? 'bg-gradient-to-b from-amber-400 to-yellow-600 text-slate-950 border-white shadow-[0_0_15px_rgba(255,215,0,0.8)]'
                    : 'bg-slate-950/90 border-slate-700/80 text-slate-300 hover:border-slate-500'
                }`}
        ...
        {/* 3. DIFFICULTY & ROUNDS */}
        <div className="chamfer-hud-center p-4 rounded-3xl flex flex-col gap-3">
          <div>
            ...
            <div className="grid grid-cols-3 gap-1.5">
              {difficulties.map((d) => (
                <button
                  ...
                  className={`tactile-btn py-1.5 px-1 rounded-xl text-xs font-black font-game border-2 transition-all cursor-pointer ${
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.94 }}
        onClick={handleStart}
        className="tactile-btn gleam-effect relative group px-14 sm:px-24 py-4 sm:py-5 rounded-3xl bg-gradient-to-r from-amber-500 via-yellow-300 to-amber-600 border-4 border-yellow-100 text-slate-950 font-black text-2xl sm:text-3xl md:text-4xl tracking-wider uppercase font-game shadow-[0_0_60px_rgba(255,215,0,0.9),0_12px_30px_rgba(0,0,0,0.95),inset_0_2px_10px_rgba(255,255,255,0.9)] flex items-center gap-4 cursor-pointer mb-2 overflow-hidden"
      >
