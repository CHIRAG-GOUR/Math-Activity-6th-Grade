...
export const VictoryCinematic: React.FC<VictoryCinematicProps> = ({
  ...
  useEffect(() => {
    ...
    // Trigger celebratory gold & team confetti cannon
    const count = 250;
    const defaults = {
      origin: { y: 0.65 },
      colors: blueWon
        ...
        : redWon
        ? ['#FF2A5F', '#FFD700', '#FFFFFF', '#FF5500']
        : ['#FFD700', '#00F0FF', '#FF2A5F', '#FFFFFF'],
    };
  ...
  return (
    <div className="relative w-full h-full flex flex-col items-center justify-center p-4 sm:p-6 lg:p-8 z-40 select-none max-w-5xl mx-auto overflow-y-auto">
      {/* Background Volumetric Gold Light Flood */}
      <div className="absolute inset-0 bg-radial from-amber-500/30 via-black/85 to-black/98 pointer-events-none" />
      ...
      >
        <div className="flex items-center gap-2 px-5 py-1.5 rounded-full bg-amber-500/25 border-2 border-amber-400 text-amber-300 text-xs font-black tracking-widest uppercase font-game mb-2 shadow-[0_0_25px_rgba(255,215,0,0.7)]">
          <Sparkles className="w-4 h-4 text-amber-300" />
          <span>VAULT CRACKED • HEIST COMPLETE</span>
        </div>

        <h1 className="text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-black font-game uppercase tracking-tight text-white drop-shadow-[0_10px_40px_rgba(0,0,0,1)]">
          {blueWon ? (
            <span className="blue-neon-text">{teamBlue.name} WINS!</span>
          ) : redWon ? (
            <span className="red-neon-text">{teamRed.name} WINS!</span>
          ) : (
            <span className="gold-metallic-text">EPIC DRAW!</span>
          )}
        </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full mb-8 z-10">
        ...
        <motion.div
          ...
          className={`p-6 rounded-3xl backdrop-blur-md flex flex-col justify-between ${
            blueWon
              ? 'chamfer-hud-blue ring-4 ring-cyan-300 shadow-[0_0_60px_rgba(0,229,255,0.6)]'
              : 'bg-slate-950/85 border border-slate-700 opacity-80'
          }`}
        >
          <div className="flex items-center justify-between border-b border-slate-700/80 pb-3 mb-4">
            ...
            {blueWon && (
              <div className="flex items-center gap-1.5 px-4 py-1 rounded-full bg-gradient-to-r from-amber-400 to-yellow-300 text-slate-950 font-black text-xs uppercase font-game shadow-[0_0_15px_rgba(255,215,0,0.9)]">
          ...
          <div className="grid grid-cols-2 gap-4 my-2">
            <div className="flex flex-col">
              <span className="text-xs font-black text-slate-400 uppercase font-game">FINAL SCORE</span>
              <span className="text-5xl font-black text-white font-display">{teamBlue.score}</span>
            </div>
            <div className="flex flex-col">
              <span className="text-xs font-black text-slate-400 uppercase font-game">KEYS COLLECTED</span>
              <div className="flex items-center gap-2 text-3xl font-black text-amber-300 font-game">
        ...
        <motion.div
          ...
          className={`p-6 rounded-3xl backdrop-blur-md flex flex-col justify-between ${
            redWon
              ? 'chamfer-hud-red ring-4 ring-rose-300 shadow-[0_0_60px_rgba(255,42,95,0.6)]'
              : 'bg-slate-950/85 border border-slate-700 opacity-80'
          }`}
        >
          <div className="flex items-center justify-between border-b border-slate-700/80 pb-3 mb-4">
            <div className="flex items-center gap-2.5">
              <div className="w-4 h-4 rounded-full bg-rose-400 shadow-[0_0_12px_#ff2a5f]" />
      ...
      <div className="flex items-center gap-5 z-10">
        <motion.button
          ...
          className="tactile-btn gleam-effect px-10 sm:px-14 py-4 rounded-2xl bg-gradient-to-r from-amber-500 via-yellow-300 to-amber-600 border-2 border-yellow-100 text-slate-950 font-black text-lg sm:text-xl uppercase font-game shadow-[0_0_45px_rgba(255,215,0,0.9)] flex items-center gap-3 cursor-pointer"
        >
